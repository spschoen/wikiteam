...
def getall():
    ...
    while True:
        wiki_list = getlist(wikia, offset, offset + limit)
        if wiki_list:
            print(offset)
            domains = dict(domains.items() + wiki_list.items())
            empty = 0
        else:
            empty += 1
...
def main():
    domains = getall()
    with open('../mediawiki/wikia.com', 'w') as out:
        out.write('\n'.join(str(domains[i]['domain']) for i in domains))
    
    # TODO: Remove the following code entirely. All Wikia wikis can now be assumed to be un-dumped.
    return
